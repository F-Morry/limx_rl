...
#from omni.isaac.lab.managers.commands.velocity_commands import UniformVelocityCommandCfg
#from isaaclab.managers.commands.velocity_commands import UniformVelocityCommandCfg
from dataclasses import dataclass

from bipedal_locomotion.tasks.locomotion.mdp import UniformVelocityCommandCfg
...
##############
@dataclass
class ExtendedRanges(UniformVelocityCommandCfg.Ranges):
    lin_vel_z: tuple[float, float] = (0.0, 0.0)  # 添加字段

class ExtendedUniformVelocityCommandCfg(mdp.UniformVelocityCommandCfg):
    ranges: ExtendedRanges = ExtendedRanges()  # 使用扩展 Ranges
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=ExtendedRanges(
            lin_vel_x=(-0.7, 0.7), lin_vel_y=(-0.5, 0.5), 
            # 修改处：新增 lin_vel_z=(0.0, 4.0)，用于生成垂直跳跃速度指令，这是跳跃动作的核心，允许机器人接收随机向上速度目标（0~4 m/s），以训练起跳能力
            lin_vel_z=(0.0, 4.0), 
...
class RewardsCfg:
    ...
    # 修改处：新增 rew_lin_vel_z 奖励项，weight=2.5，用于鼓励机器人追踪垂直速度指令（lin_vel_z），这是跳跃训练的关键正向奖励，帮助策略学习向上加速
    rew_lin_vel_z = RewTerm(
        func=mdp.track_lin_vel_z_exp, weight=2.5, params={"command_name": "base_velocity", "std": math.sqrt(0.25)}
    )
    # 修改处：新增 rew_flight_time 奖励项，weight=3.0，使用 no_fly 函数（假设项目已有，用于奖励轮子离地时间），鼓励空中飞行阶段，强化跳跃的持续性
    rew_flight_time = RewTerm(
        func=mdp.no_contact, weight=3.0,
    ...
    # penalizations
    # 修改处：调整 pen_lin_vel_z 的权重从 -0.3 到 -0.1，减少对垂直速度的惩罚力度，允许机器人尝试更大向上速度而不受重罚，促进跳跃探索
    ...
    # 修改处：调整 pen_base_height 的权重从 -30.0 到 -10.0，减少对基座高度的惩罚，允许跳跃时高度超过目标（0.80m），避免抑制向上运动
    pen_base_height = RewTerm(func=mdp.base_com_height, params={"target_height": 0.80}, weight=-10.0)
...
@configclass
# 修改处：将主环境类从 WFEnvCfg 改为 JumpingEnvCfg，用于区分跳跃任务配置，便于注册和调用新任务（如 --task=Isaac-WF-Jumping-v0）
