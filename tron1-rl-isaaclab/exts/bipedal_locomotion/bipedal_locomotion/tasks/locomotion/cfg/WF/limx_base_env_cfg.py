...
class RewardsCfg:
    ...
    stand_still = RewTerm(
        ...
        weight=-5.2 #-5.0 to -5.2
    )

    # rewards
    rew_lin_vel_xy = RewTerm(
        #std0.2 to 0.1，强化追踪命令的奖励，鼓励机器人更好地跟踪速度命令，提升站立和小范围移动的能力
        func=mdp.track_lin_vel_xy_exp, weight=2.0, params={"command_name": "base_velocity", "std": math.sqrt(0.08)}
    )
    # 追踪角速度z，但不鼓励旋转，保持站立，调大权重增加yaw_vel惩罚
    rew_ang_vel_z = RewTerm(
        func=mdp.track_ang_vel_z_exp, weight=2.0, params={"command_name": "base_velocity", "std": math.sqrt(0.08)} #std:0.25 to 0.08
    )
    ...
    rew_same_foot_x_position = RewTerm(
        ...
        weight=-35, #-20 to -30 # 0.1, # changed to penalty mode
        params={"asset_cfg": SceneEntityCfg("robot", body_names="wheel_.*")},
    )
    ...
    pen_flat_orientation_l2 = RewTerm(func=mdp.flat_orientation_l2, weight=-18.0) #-12.0 to -15.0
