...
from bipedal_locomotion.tasks.locomotion.cfg.WF.limx_jumping_env_cfg import JumpingEnvCfg 
...
@configclass
class WFBaseJumpingEnvCfg(JumpingEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        self.scene.robot = WHEELFOOT_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        self.scene.robot.init_state.joint_pos = {
            ...
            "knee_L_Joint": 0.0,
            "knee_R_Joint": 0.0,
            "wheel_L_Joint": 0.0,  # 添加：确保轮子初始化为0
            "wheel_R_Joint": 0.0,
        }
        self.events.add_base_mass.params["asset_cfg"].body_names = "base_Link"
        self.events.add_base_mass.params["mass_distribution_params"] = (0.0, 2.0)  # 改为正值，避免负质量
        self.terminations.base_contact.params["sensor_cfg"].body_names = "base_Link"
...
@configclass
class WFBaseJumpingEnvCfg_PLAY(WFBaseJumpingEnvCfg):
    def __post_init__(self):
        super().__post_init__()
...
#############################
# Wheelfoot Jumping Environment
#############################

@configclass
class WFJumpingEnvCfg(WFBaseJumpingEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        
        # Set flat terrain for jumping (no rough or stairs)
        self.scene.terrain.terrain_type = "plane"
        
        # Disable height scanner if not needed for jumping (similar to blind flat)
        self.scene.height_scanner = None
        self.observations.policy.heights = None
        self.observations.critic.heights = None
        
        # Adjust commands for jumping focus: limit horizontal, emphasize vertical
        self.commands.base_velocity.ranges.lin_vel_x = (0.0, 0.0)  # Minimal horizontal movement
        self.commands.base_velocity.ranges.lin_vel_y = (0.0, 0.0)
        self.commands.base_velocity.ranges.ang_vel_z = (0.0, 0.0)  # No turning
        # Vertical range already set in JumpingEnvCfg (0.0, 4.0)
        
        # No terrain curriculum for jumping
        self.curriculum.terrain_levels = None

@configclass
class WFJumpingEnvCfg_PLAY(WFBaseJumpingEnvCfg_PLAY):  # Inherit from WFBaseJumpingEnvCfg for play mode
    def __post_init__(self):
        super().__post_init__()
        ...
        # Fixed commands for demo (e.g., consistent jump height)
